...
# - [PyCharm Debugger](https://www.jetbrains.com/help/pycharm/debugging-your-first--application.html)
...
# The function returns None because it lacks a return statement. When a function performs a calculation but does not explicitly return the result, 
# Python automatically returns None by default. In this case, the function computes the sum of two numbers and stores it in a variable, 
# but since there is no return statement, the result is not passed back to the caller. When the function is called within a print statement,
# it outputs None instead of the expected sum. To fix this, a return statement must be added so that the computed value is properly returned and displayed.
...
# Normally, dividing by zero (10 / 0) would raise a ZeroDivisionError, but since the except block is present without specifying 
# an exception type, it catches all exceptions silently. The pass statement means "do nothing," so the program continues execution 
>>>>>>> bdecb77047b488528f699166a2507503f451dca2
